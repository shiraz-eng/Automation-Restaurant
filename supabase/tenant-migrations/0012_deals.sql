-- ============================================================================
-- Tenant delta 0012 — P7: Deals & Combos
--
--   * deals + deal_components. order_lines.deal_id.
--   * place_order accepts { deal_id, qty } lines: writes a priced HEADER line
--     (deal_id set, no item/variant) + zero-priced COMPONENT lines (real
--     item/variant, deal_id set) so the kitchen sees the parts and recipe stock
--     still deducts per component. Order subtotal = header prices only.
--   * RLS (deals.view / deals.update; guest reads live deals) + audit + realtime.
--
-- Depends on 0006 (deals.* keys) + 0010 (8-arg place_order). Idempotent.
-- ============================================================================
...
create table if not exists public.deal_components (
  id           uuid primary key default gen_random_uuid(),
...
create index if not exists deal_components_deal_idx on public.deal_components(deal_id, sort_order);

alter table public.order_lines add column if not exists deal_id uuid references public.deals(id) on delete set null;

-- ── place_order: deal-aware ──────────────────────────────────────────────
...
drop policy if exists staff_read on public.deals;
drop policy if exists mgr_write on public.deals;
drop policy if exists guest_read on public.deals;
drop policy if exists staff_read on public.deal_components;
drop policy if exists mgr_write on public.deal_components;
drop policy if exists guest_read on public.deal_components;
...
do $$ begin execute 'alter publication supabase_realtime add table public.deals';
exception when duplicate_object then null; end $$;

do $$
declare tbl text;
begin
  foreach tbl in array array['deals','deal_components'] loop
