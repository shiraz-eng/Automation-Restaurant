-- 0026: Build-Your-Own-Combo (spec §7-10) — selectable option groups on
-- top of a deal's fixed components/price ("Choose 1 Main", "Choose 1
-- Drink"), with upcharges. A deal with no option groups configured is
-- unaffected — this is additive to the existing fixed-price combo model
-- (deals/deal_components), not a replacement for it.
...
create index if not exists deal_option_groups_deal_idx on public.deal_option_groups(deal_id, sort_order);
...
create index if not exists deal_option_items_group_idx on public.deal_option_items(group_id, sort_order);
...
drop policy if exists staff_read on public.deal_option_groups;
drop policy if exists mgr_write on public.deal_option_groups;
drop policy if exists guest_read on public.deal_option_groups;
...
drop policy if exists staff_read on public.deal_option_items;
drop policy if exists mgr_write on public.deal_option_items;
drop policy if exists guest_read on public.deal_option_items;
...
-- place_order(): same body as before, plus the Build-Your-Own selection
-- block (client sends deal_option_items IDs only, re-priced/re-validated
-- server-side exactly like item modifiers, one group at a time).
