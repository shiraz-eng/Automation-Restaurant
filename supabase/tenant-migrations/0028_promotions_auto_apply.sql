-- 0028: Auto-apply promotions. Every promotion so far (percent/fixed, even
-- scheduled ones) only ever discounted an order when a customer typed a
-- matching code — a scheduled "happy hour" promo had no way to actually
-- fire on its own. This adds an auto_apply flag and best_auto_promotion(),
-- and place_order() now applies the single best eligible auto_apply
-- promotion itself whenever nothing else already discounted the order.

alter table public.promotions add column if not exists auto_apply boolean not null default false;

-- promo_discount() (unchanged from 0027) and the new best_auto_promotion():
...
-- place_order(): same body as before, plus the auto-apply fallback when
-- nothing else already discounted the order.
